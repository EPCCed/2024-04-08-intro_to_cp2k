...
class Structures(MutableSequence):
    ...
    def random_split(self, size = [0.9, 0.1], seed = 9):
        """Randomly sample structures without replacement.
        ...
        """
        np.random.seed(seed)
        p1_size = int(len(self._list) * size[0]) if (isinstance(size[0], float) and size[0] < 1) else int(size[0])
        choice = np.random.choice(len(self._list), p1_size, replace=False)
        keep = np.zeros(len(self._list), dtype = bool)
        keep[choice] = True
        rest = ~keep
        idx = np.arange(len(self._list))
        p1_idx, p2_idx = idx[keep], idx[rest]
        return self.get(p1_idx), self.get(p2_idx)
