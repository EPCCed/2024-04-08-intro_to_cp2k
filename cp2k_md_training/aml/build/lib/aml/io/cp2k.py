...
def read_frames_cp2k(fn_positions, cells = None, fn_forces = None, read_energy: bool = True, force_unit = 1.0):
    ...
    if cells is not None and len(cells.shape) == 2:
        ...
        frames.append(repeat(Frame(cell = cells)))
    
    elif cells is not None and len(cells.shape) == 3:
        frames.append((Frame(cell = cell) for cell in cells))
